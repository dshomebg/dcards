...
- SHP-2b: изтриване на продукт/вариант с поръчки удря `restrict` и дава „опитай пак" вместо ясно
  съобщение — SHP-3 да го прави `isActive=false`. Лимитът по имейл се харчи и при `out_of_stock`;
  3 заявки с чужд имейл го заключват за час (IP лимитът остава). Годината в номера е по UTC.
- SHP-2c: имейлът от формата е непроверен — писмото да не носи телефон/адрес, само номер + линк.
