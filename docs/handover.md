...
миграция `0007`). Етап 3 е завършен; етап 4 е започнат.** Писмата при регистрация и смяна на парола, CRD-3 (`/c` route handler) и AUTH-12
(потвърждение на имейл) са готови. Остава: картово плащане (решение за доставчик).
...
2. Картово плащане — първо собственикът избира доставчик (Stripe / myPOS / друг). Кандидати
   междувременно: дизайн/полиране на витрината и профилите, `/admin/users` и `/admin/orgs` (ако са
   още предстоящи), качване на лого в персонализацията (zadanie 6.3).
