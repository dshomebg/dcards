...
**Комитнати: етап 0, `ADM-1`, `PLT-1..3`. `PLT-4` е готов локално и чака комит.** Нищо не е
деплойвано. Следва PLT-5 — редакторът на профил (`/app/profiles/{id}`: полета, линкове с ред,
тема, живо превю) — с него етап 1 приключва.
